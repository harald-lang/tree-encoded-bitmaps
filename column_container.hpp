...
namespace cont{
  ...
  struct cont_elem{
    ...
    void print(){
      switch(size){
        case 8:
          cout << to_string(*(reinterpret_cast<$u8*>(ptr)));
          break;
        case 16:
          cout << to_string(*(reinterpret_cast<$u16*>(ptr)));
          break;
        case 32:
          cout << to_string(*(reinterpret_cast<$u32*>(ptr)));
          break;
        case 64:
          cout << to_string(*(reinterpret_cast<$u64*>(ptr)));
          break;
        default:
          cout << "Unsupported data type! (in cont_elem.print())";
      }
    }
  };
  ...
  struct container{
    ...
    virtual pair<bool, u8> dataType()=0;
...
*/
  ...
  unique_ptr<container> storeColumn($u8 *col, $u64 num_elements){
    container* column = new $u8_column(col, num_elements);
    return unique_ptr<container>(column);
  }

  unique_ptr<container> storeColumn($u16 *col, $u64 num_elements){
    container* column = new $u16_column(col, num_elements);
    return unique_ptr<container>(column);
  }

  unique_ptr<container> storeColumn($u32 *col, $u64 num_elements){
    container* column = new $u32_column(col, num_elements);

    return unique_ptr<container>(column);
  }

  unique_ptr<container> storeColumn($u64 *col, $u64 num_elements){
    container* column = new $u64_column(col, num_elements);
    return unique_ptr<container>(column);
  }

  unique_ptr<container> storeColumn($i8 *col, $u64 num_elements){
    container* column = new $i8_column(col, num_elements);
    return unique_ptr<container>(column);
  }

  unique_ptr<container> storeColumn($i16 *col, $u64 num_elements){
    container* column = new $i16_column(col, num_elements);
    return unique_ptr<container>(column);
  }

  unique_ptr<container> storeColumn($i32 *col, $u64 num_elements){
    container* column = new $i32_column(col, num_elements);
    return unique_ptr<container>(column);
  }

  unique_ptr<container> storeColumn($i64 *col, $u64 num_elements){
    container* column = new $i64_column(col, num_elements);
    return unique_ptr<container>(column);
  }

  //applies the permutation to an array for further tasks
  void applyPermutation(vector<vector<cont::cont_elem>> rel_perm, vector<char*> new_rel){
      ...
      for(auto att = 0; att < rel_perm[row].size(); att++){
        switch(rel_perm[row][att].size){
          ...
          case 64:
            ...
            break;
          default:
            cout << "Unsupported data type! applyPermutation(...)" << endl;
        }
      }
    }
  }
