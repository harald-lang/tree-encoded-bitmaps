...
using rank_types_under_test = ::testing::Types<
    dtl::rank1_surf<word_type, inclusive, 512>,
    dtl::rank1_surf<word_type, exclusive, 512>,
    dtl::rank1_surf<word_type, inclusive, 256>,
    dtl::rank1_surf<word_type, exclusive, 256>,
    dtl::rank1_surf<word_type, inclusive, 128>,
    dtl::rank1_surf<word_type, exclusive, 128>,
    dtl::rank1_surf<word_type, inclusive, 64>,
    dtl::rank1_surf<word_type, exclusive, 64>>;
//===----------------------------------------------------------------------===//
...
TYPED_TEST(rank_test, random_bitmaps) {
  ...
  for (std::size_t rep = 0; rep < 5; ++rep) {
    ...
    for (std::size_t i = 0; i < 4096; ++i) {
      const auto r = rank(i, b.data());
      const auto l = linear_rank(i, b, R::is_inclusive);
      ASSERT_EQ(r, l)
          << "bitmap: " << b << "\n"
          << "test failed for i=" << i << "."
          << std::endl;
