...
bitmap_db::init() {
  ...
  // Prepare the SQL statements.
  {
    const std::string sql_stmt =
        "insert into bitmaps (n, d_num, d, d_actual, f_num, f, f_actual, bitmap)\n"
        "  values (:n, :d_num, :d, :d_actual, :f_num, :f, :f_actual, :bitmap )";
  ...
  }
  {
    const std::string sql_stmt =
        ...
        " where n = :n and f_num = :f_num and d_num = :d_num";
...
//===----------------------------------------------------------------------===//
i64
bitmap_db::store_bitmap(u64 n, num f, num d, const dtl::bitmap& b) {
  ...
  sqlite3_bind_int64(insert_stmt_,
      sqlite3_bind_parameter_index(insert_stmt_, ":d_num"), d.getRaw());
  sqlite3_bind_double(insert_stmt_,
      sqlite3_bind_parameter_index(insert_stmt_, ":d"), static_cast<f64>(d));
  sqlite3_bind_double(insert_stmt_,
      sqlite3_bind_parameter_index(insert_stmt_, ":d_actual"), d_actual);
  sqlite3_bind_double(insert_stmt_,
      sqlite3_bind_parameter_index(insert_stmt_, ":f_num"), f.getRaw());
  sqlite3_bind_double(insert_stmt_,
      sqlite3_bind_parameter_index(insert_stmt_, ":f"), static_cast<f64>(f));
  sqlite3_bind_double(insert_stmt_,
...
bitmap_db::load_bitmap(i64 id) {
  ...
  while((rc = sqlite3_step(select_by_id_stmt_)) == SQLITE_ROW) {
    u8* blob = reinterpret_cast<u8*>(
        sqlite3_column_blob(select_by_id_stmt_, 8));
    const auto bytes = sqlite3_column_bytes(select_by_id_stmt_, 8);
...
//===----------------------------------------------------------------------===//
std::vector<$i64>
bitmap_db::find_bitmaps(u64 n, num f, num d) {
  std::lock_guard<std::mutex> lock(mutex_);
  // Critical section.
  ...
  sqlite3_bind_double(select_ids_stmt_,
      sqlite3_bind_parameter_index(select_ids_stmt_, ":d_num"), d.getRaw());
  sqlite3_bind_double(select_ids_stmt_,
      sqlite3_bind_parameter_index(select_ids_stmt_, ":f_num"), f.getRaw());
