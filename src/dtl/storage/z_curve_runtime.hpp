...
//naive version
template<typename T>
bool cmp_coord_helper(const std::vector<cont::cont_elem>& tpl1,
                      const std::vector<cont::cont_elem>& tpl2,
                      const std::vector<$u32>& attr_idx_cmp) {
  $u16 j = 0;
  $u64 x = 0;

  $u16 numAttr = std::min(tpl1.size(), tpl2.size());
  ...
  for (auto i : attr_idx_cmp) { //determine the data type for padding
    ...
    switch(tpl1[i].size){
      ...
      case 64:
        ...
        break;
      default:
        std::cout << "Unsupported data type in cmp_coord_helper" << std::endl;
        exit(1);
    }
...
bool calc_z_curve(const std::vector<cont::cont_elem>& tpl1,
                  const std::vector<cont::cont_elem>& tpl2,
                  const std::vector<$u32>& attr_idx_cmp) {
  $u8 max_col_size = 0;
...
//    for ($u16 i = 0; i < numAttr; i++) { //determine the data type for padding
  for (auto i : attr_idx_cmp) { //determine the data type for padding
    ...
    if (std::max(t1_type,t2_type) > max_col_size)
  ...
  switch (max_col_size) {
    ...
    case 64:
      ...
      break;
    default:
      std::cout << "Error, datatype not supported! (in calc_z_curve)" << std::endl;
      exit(1);
  }
...
struct Compare{
  ...
  bool operator()(const std::vector<cont::cont_elem>& tpl1, const std::vector<cont::cont_elem>& tpl2) const {
    return calc_z_curve(tpl1,tpl2, attr_idx_cmp);
  }
};

template <typename T>
bool cmp_coord_helper(const std::vector<cont::container*>& tuples, u64 first_tuple, u64 second_tuple){

  if(first_tuple == second_tuple) {
    std::cout << "Just compare different tuples! (first_tuple != second_tuple)!" << std::endl;
    exit(1);
  }
...
bool calc_z_curve(const std::vector<cont::container*>& tuples, u64 first_tuple, u64 second_tuple) {
  $u8 max_col_size = 0;

  for ($u8 i = 0; i < tuples.size(); i++) { //determine the data type for padding
    std::pair<bool, $u8> t1_type = tuples[i]->dataType();
  ...
  switch (max_col_size) {
    ...
    case 64:
      ...
      break;
    default:
      std::cout << "Error, datatype not supported!" << std::endl;
      exit(1);
  }
...
bool calc_z_curve_diff_signed(const std::vector<cont::container*>& tuples, u64 first_tuple, u64 second_tuple) {
