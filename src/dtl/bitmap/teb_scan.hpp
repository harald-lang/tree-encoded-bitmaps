...
class teb_scan<optimization_level_>::iter {
...
public:
  ...
  iter(const teb_scan& teb) noexcept :
      teb_(teb),
...
/// 1-fill iterator, with skip support.
template<i32 optimization_level_>
class teb_scan<optimization_level_>::scan_iter {
...
public:
  ...
  /// the iterator points to the first 1-fill.
  explicit __teb_inline__
  scan_iter(const teb_scan& teb, u64 batch_size = DEFAULT_BATCH_SIZE) noexcept
      : teb_(teb),
        tree_height_(determine_tree_height(teb.n_)),
        perfect_levels_(
            determine_perfect_tree_levels(teb_.implicit_inner_node_cnt_)),
